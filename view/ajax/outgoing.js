$(document).ready(function () {
    ...
    function mostrarProveedor() {
        $.ajax({
            url: "http://localhost:3000/api/v1/proveedor",
            method: "GET",
            success: function (data) {
                var select = $("#proveedor-select");
                data.sort(function (a, b) {
                    return a.id - b.id;
                });
                $.each(data, function (index, item) {
                    select.append($("<option>").val(item.id).text(item.razon_social + " " + estado));
                });
    ...
    $("#proveedor-select").on("change", function () {
        ...
        $.ajax({
            url: "http://localhost:3000/api/v1/contactos/clientes/" + clienteId,
            method: "GET",
            success: function (cliente) {
                ...
                $("#cliente-categoria").text(cliente.categoria_cliente.nombre);
                $("#cliente-porcentaje_descuento").text(
                    cliente.categoria_cliente.porcentaje_descuento
                );
            },
        });
    });
